...
**The anti-cliché rule:** a green-on-black LED clock is *exactly* the "near-black + single acid-green accent" look the `frontend-design` skill flags as generic AI. So green is **scoped to the LED display only**; the identity is spent instead on a **multi-LED scoreboard** palette, segmented DSEG typography with ghost segments, and a tactile buzzer. The free design axes don't get spent on a default.
...
2. **LED power-on** — the wordmark and clock *boot* like hardware: ghost segments, a flicker warm-up, bloom; the clock ignites (digits running up) then fades to dark each interval.
